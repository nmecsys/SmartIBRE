...
api_ibge<-function(linhas){
  ...
  for (i in 1:length(p2)){
    ...
    x=subset(frame_periodo,frame_periodo[,1]==dados[linhas[i],5])
    t=x[,2]
    
    if (b==t || i<2){
      url<-paste0(p1,p2[i],p3)
      y<-try(download.file(url,paste0("data/","elegante.csv")))
      ...
      cat(p2[i])
      api<-read.csv(paste0("data/","elegante.csv"),sep="\t",dec=",") # serie do IBGE com cabeçalho   
      api_t<-read.csv(paste0("data/","elegante.csv"),sep="\t",header=FALSE,dec=",")# serie do IBGE sem cabeçalho 
      
      if (dados[linhas[i],5]=="Mensal"){
      ...
      }else if (dados[linhas[i],5]=="Ano_mes"){
      ...
      }else if(dados[linhas[i],5]=="Anual"){
        if(as.character(api_t[1,2])=="OPCAO"){
          api<-api[,-1]
          api2<-data.frame(t(api[,-c(1:2)]))
          names(api2)<-api[,1]
      ...
      }else if(dados[linhas[i],5]=="Trimestral"){
        if(as.character(api_t[1,2])=="OPCAO"){
          api<-api[,-1]
          api2<-data.frame(t(api[,-c(1:2)]))
          names(api2)<-api[,1]
  ...
  list(apit=apit,ts=ts,codigo=p2,nome=nome,data=data_ibge,descricao=as.character((dados[linhas,2])))
