#library(SSOAP)
#source("wsdlProcess.R")
library(XML)
library(RCurl)
library(XML2R)
library(zoo)

getSeries <- function(codigos, data.ini = "31/12/1990", remove.old = TRUE,periodicidade="TRI"){
  
  df_periodo<-data.frame(periodicidade=c("M","A","TRI"),periodo=c(12,1,4))
  ...
  else if(periodicidade=="TRI"){
    
    df_trimestre<-data.frame(mes=c(1:12),trimestre=c(1,1,1,2,2,2,3,3,3,4,4,4))
    
    if(length(unique(df[,3]))==1){
      start_ano<-as.numeric(substr(df[1,1],1,4))
      start_t<-df_trimestre[which(df_trimestre$mes==as.numeric(substr(df[1,1],6,7))),2]
      
      minha_ts<-ts(df[,2],start<-c(start_ano,start_t),freq=df_periodo[df_periodo$periodicidade==periodicidade,2])
      ...
      df_cortado<-df[df$serie==cods[1],]
      start_ano<-as.numeric(substr(df_cortado[1,1],1,4))
      start_t<-df_trimestre[which(df_trimestre$mes==as.numeric(substr(df_cortado[1,1],6,7))),2]
      minha_ts<-ts(df_cortado[,2],start<-c(start_ano,start_t),freq=df_periodo[df_periodo$periodicidade==periodicidade,2])
      ...
      for(i in 2:length(cods)){
        ...
        start_ano<-as.numeric(substr(df_cortado[1,1],1,4))
        start_t<-df_trimestre[which(df_trimestre$mes==as.numeric(substr(df_cortado[1,1],6,7))),2]
        minha_ts<-cbind(minha_ts,ts(df_cortado[,2],start<-c(start_ano,start_t),freq=df_periodo[df_periodo$periodicidade==periodicidade,2]))
      }}}
